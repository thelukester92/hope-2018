(function($, undefined)
{
    $(function()
    {
        $(".hope-accordion > h2, .hope-accordion > h3").click(function()
        {
        ...
        {
            var el = $("#twitter-widget-0");
            if(el.length > 0 && el.contents().find(".timeline-Tweet").length > 0)
                el.contents().find("head").append(
                    "<style>" +
                    ".timeline-Tweet:hover, .timeline-Tweet-media * { background: none; }" +
                    ...
                    ".TweetAuthor-name:hover, .timeline-Tweet a:hover { color: #fff; text-decoration: underline; }" +
                    "</style>"
                );
            else
                setTimeout(hackTwitter, 50);
        }
